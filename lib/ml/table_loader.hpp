...
private:
    ...
    template<typename T> 
    bool preparse(int col_index, T*){return true;} //do nothing 
    ...
    template<typename T> 
    bool postparse(int col_index, T*){return true;} //do nothing
...
TableLoader<colTypes...>::~TableLoader(){
    for (int i=0;i<col_num;++i){
        if (col_dim_aggs[i]) delete col_dim_aggs[i];
    };
}
...
bool TableLoader<colTypes...>::postparse(int col_index, husky::lib::SparseVectorXd*){
    if (col_dims[col_index]<=0){
        ...
        husky::LOG_I << "Resize sparse column " << col_index << " to " << col_dim_aggs[col_index]->get_value() << std::endl;
    }
    return true;
}
...
bool TableLoader<colTypes...>::parse_col(int col_index, T& defaultVal){
    if (t_it==t_end) return false;
    T val; 
    std::stringstream ss(*t_it);
    ss >> val;
    if (!ss){
        if (allow_parse_failure){ val = defaultVal; }
        else {throw base::HuskyException("Parse failure for column " + col_names[col_index] );}
    }
...
bool TableLoader<colTypes...>::parse_col(int col_index, husky::lib::VectorXd& defaultVec){
    ...
    if (col_dims[col_index]<=0){
        ...
        for (int i=0;i<col_dim;i++){ vec[i] = tmpvec[i]; };
    }else{
        ...
        for (int i=0;i<col_dims[col_index];i++){
            ...
            ss >> tmpval;
            if (!ss){
                if (allow_parse_failure && i<defaultVec.size() ){ tmpval = defaultVec[i]; }
                else {throw base::HuskyException("Parse failure for column " + col_names[col_index] );}
            }
            vec[i] = tmpval;
        };
...
bool TableLoader<colTypes...>::parse_col(int col_index, husky::lib::SparseVectorXd& defaultVec){
    ...
    while( t_it != t_end){
        ...
        if (splitPos == std::string::npos) break;

        bool success = true;

        ss.clear(); ss.str( t_it->substr(0,splitPos) ); 
        int idx; ss>>idx;
        if (!ss) success = false;

        ss.clear(); ss.str( t_it->substr(splitPos+1, t_it->length()-splitPos) );
        double tmpval; ss >> tmpval;
        if (!ss) success = false;
        ...
        if (idx+1>=col_dims[col_index]){
            if (col_dims[col_index]>0){
                ...
                throw husky::base::HuskyException("vector dimension larger then user specified for column " + col_names[col_index] );
            }else{
...
void TableLoader<colTypes...>::set_col_names( std::vector<std::string> col_names) {
    if (col_names.size()!=col_num) throw base::HuskyException("Mismatch in number of columns");
    this->col_names = col_names;
}

template<typename ... colTypes> 
void TableLoader<colTypes...>::set_col_dim( int col_index, int dim) {
    if (col_index>=col_num || col_index<0) throw base::HuskyException("Col index out of range");
