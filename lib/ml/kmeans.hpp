...
class Kmeans {
   public:
    ...
    Kmeans<FeatureVec>& set_k(int k) {
        this->k = k;
        return *this;
    }
    Kmeans<FeatureVec>& set_iter(int iter) {
        this->iter = iter;
        return *this;
    }
    Kmeans<FeatureVec>& set_init_opt(KmeansOpts opt) {
        this->kInitOption = opt;
        return *this;
    }

    // options related to Kmeans||
    Kmeans<FeatureVec>& set_center_sample_n_iter(int n) {
        this->centerSampleNIter = n;
        return *this;
    }
    Kmeans<FeatureVec>& set_center_sample_per_iter(int n) {
        this->centerSamplePerIter = n;
        return *this;
    }
    ...
    const std::vector<FeatureVec>& get_centers() {
        if (!trained)
            throw base::HuskyException("Kmeans not yet trained");
   ...
   private:
    ...
    std::vector<ColT> get_random_rows_col_val(ObjList<ObjT>& obj_list, AttrList<ObjT, ColT>& target_col, int count,
                                              std::function<double()>& dice, AttrList<ObjT, double>* weight_col = NULL);
    ...
    if (clusterCenters.size() == 0) {  // init_center not provided
        ...
        if (kInitOption == KmeansOpts::kInitSimple) {
            ...
            if (kInitOption == KmeansOpts::kInitKmeansBarBar) {
                if (centerSampleNIter * centerSamplePerIter < k)
                    throw husky::base::HuskyException("KmeansBarBar parameter error, sampled center count less then k");
            }
            ...
            for (int i = 0; i < centerSampleNIter; i++) {
                ...
                auto newClusterCenters =
                    get_random_rows_col_val(obj_list, feature_col, centerSamplePerIter, dice, &d2_col);
                ...
                clusterCenters.insert(clusterCenters.end(), std::make_move_iterator(newClusterCenters.begin()),
...
std::vector<ColT> Kmeans<FeatureVec>::get_random_rows_col_val(ObjList<ObjT>& obj_list, AttrList<ObjT, ColT>& target_col,
                                                              int count, std::function<double()>& dice,
                                                              AttrList<ObjT, double>* weight_col) {
    ...
    if (weight_col == NULL) {
        ...
        list_execute(obj_list, {}, {&ac}, [&](ObjT& obj) { totWs[tid].update(weight_col->get(obj)); });
    }
...
void Kmeans<FeatureVec>::fit(ObjList<ObjT>& obj_list, AttrList<ObjT, FeatureVec>& feature_col) {
    if (!get_distance_sq)
        get_distance_sq = KmeansUtil::get_default_distance_sq_func((FeatureVec*) 0);
    if (!get_distance_sq) {
        husky::LOG_I << "distance function lamba not given. Abort" << std::endl;
        return;
    }
    ...
    if (tid == 0) {
        husky::LOG_I << "detected feature dim as " << dim << std::endl;
    }
    ...
    for (int i = 0; i < k; i++) {
        clusterCenterAggVec.push_back(husky::lib::Aggregator<FeatureVec>(
            KmeansUtil::get_zero<FeatureVec>(dim), [&](FeatureVec& a, const FeatureVec& b) { a += b; },
    ...
    for (int it = 0; it < iter; ++it) {
        ...
        if (switchClassCountAgg.get_value() == 0)
            break;
